...
enum EdgeDir {
  DIR_LEFT, DIR_TOP, DIR_RIGHT, DIR_BOTTOM
};
...
GameState createNewGameState(Coord fieldSize) {
  ...
  for (uint8_t iteration = 1; iteration < gs.fieldSize.x*gs.fieldSize.y; ++iteration) {
    ...
    for (uint8_t i = 0; i < 4; ++i) {
      ...
      if (newCand.x < 0 || newCand.y < 0 || newCand.x >= gs.fieldSize.x || newCand.y >= gs.fieldSize.y) {
        continue;
      }
      ...
      if (found) continue;
      candidates[num_candidates] = newCand; num_candidates++;
    }
  }
  free(candidates);
...
void recalculateReachables(GameState* gs) {
  ...
  uint8_t stackSize = 1;
  while (stackSize > 0) {
    Coord curr = stack[stackSize-1]; stackSize--;
    ...
    for (uint8_t i = 0; i < 4; ++i) {
      ...
      if (neib.x < 0 || neib.y < 0 || neib.x >= gs->fieldSize.x || neib.y >= gs->fieldSize.y) continue;
      if (gs->reachable[neib.x][neib.y] || !gs->elements[neib.x][neib.y].edges[OPP[i]]) {
        continue;
      }
      stack[stackSize] = neib; stackSize++;
    }
  }
  free(stack);
}
...
uint8_t determine_element_size(Coord fieldSize) {
  ...
  for (uint8_t i = 0; i < n; ++i) {
    if (SCREEN_WIDTH / availableSizes[i] >= fieldSize.x && SCREEN_HIGHT / availableSizes[i] >= fieldSize.y) {
...
void draw_grid(Canvas* canvas, GameState* gs, uint8_t elementSize) {
  Coord startCorner = createCoord(SCREEN_WIDTH/2 - gs->fieldSize.x * elementSize/2, SCREEN_HIGHT/2 - gs->fieldSize.y * elementSize/2);
  for (uint8_t i = 0; i < gs->fieldSize.x; ++i) {
    for (uint8_t j = 0; j < gs->fieldSize.y; ++j) {
      if (elementSize == 10) {
        draw_grid_element_size10(canvas, createCoord(startCorner.x + i*10, startCorner.y+j*10), createCoord(i, j), gs);
      } else if (elementSize == 12) {
        draw_grid_element_size12(canvas, createCoord(startCorner.x + i*12, startCorner.y+j*12), createCoord(i, j), gs);
      }
    }
  }
}
...
void draw_menu(Canvas* canvas, const MenuEntry* menu, uint8_t selectedIndex) {
  ...
  for (uint8_t i = 0; i < nitems; ++i) {
    canvas_set_color(canvas, ColorBlack);
    if (i == selectedIndex) {
      canvas_draw_box(canvas, SCREEN_WIDTH/2-max_width/2, SCREEN_HIGHT/2-item_h*nitems/2+i*item_h, max_width, item_h);
    }
    canvas_set_color(canvas, i == selectedIndex ? ColorWhite : ColorBlack);
    canvas_draw_str_aligned(canvas, SCREEN_WIDTH/2, SCREEN_HIGHT/2-item_h*nitems/2+i*item_h+item_h/2, AlignCenter, AlignCenter, menu[i].text);
  }
  canvas_set_color(canvas, ColorBlack);
  canvas_draw_rframe(canvas, SCREEN_WIDTH/2-max_width/2-DLT_FRAME, SCREEN_HIGHT/2-item_h*nitems/2-DLT_FRAME, max_width+DLT_FRAME*2, item_h*nitems+DLT_FRAME*2, 2);
}
...
void draw_about(Canvas* canvas) {
  ...
  for (uint8_t i = 0; i < nitems; ++i) {
    ...
    canvas_draw_str_aligned(canvas, SCREEN_WIDTH/2-max_width/2, SCREEN_HIGHT/2-item_h*nitems/2+i*item_h+item_h/2, AlignLeft, AlignCenter, AboutStrings[i]);
  }
}
    ...
    while(1) {
        ...
        if (appState->status == ST_PLAYING) {
          ...
          if (event.key == InputKeyBack) {
            ...
            appState->currentMenuSelection = add_delta_looped(appState->currentMenuSelection, sz, -1);
          } else if (event.key == InputKeyDown) {
            appState->currentMenuSelection = add_delta_looped(appState->currentMenuSelection, sz, 1);
          } else if (event.key == InputKeyOk) {
